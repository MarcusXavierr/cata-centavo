...
**Phase 3 — categories.** Much smaller than the ADR plans. Seed the taxonomy and the MCC map into `cache.db`, ship `setCategory` and `setCounterpartyCategory`, and the `COALESCE` chain that derives a category at read time.
*Acceptance:* "quanto gastei com mercado?" works, and correcting one transaction sticks. Correcting a merchant by CPF/CNPJ applies backwards over everything they already sold you.
